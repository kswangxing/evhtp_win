...
{
    if (thread == NULL || thread->thr == NULL) {
		printf("WX_DEBUG. INVALID PARAM.\n");
        return -1;
    }

    if (pthread_create(thread->thr, NULL, _evthr_loop, (void *)thread)) {
		printf("WX_DEBUG. failed to pthread_create.\n");
        return -1;
    }
...
{
#if 0  // Windows没有ioctl函数，所以将此功能注释掉
    int backlog = 0;
...
#ifdef EVTHR_SHARED_PIPE
    if (evutil_socketpair(/*AF_UNIX*/AF_INET, /*SOCK_DGRAM*/SOCK_STREAM, 0, fds) == -1) {
		printf("WX_DEBUG. failed to evutil_socketpair.\n");
        return NULL;
    }
